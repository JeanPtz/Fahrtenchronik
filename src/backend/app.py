...
from RoutesRepository import RoutesRepository
...
def create_route():
    try:
        routesRepository.create_routes()
...
def get_route():
    ...
    requestedRoutes = routesRepository.get_routes(route_data['name'], route_data['kfz'], route_data['start_date'], route_data['end_date'], )
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(database): # If it doesn't exist, create the database directory if needed
        ...
        if not os.path.exists(db_dir):
            os.makedirs(db_dir)
    routesRepository = RoutesRepository(database)
